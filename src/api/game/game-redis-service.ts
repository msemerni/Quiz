...
const setIsSendToSQS = async (gameUUID: string, redisClient: ReturnType<typeof createClient>): Promise<void> => {
  const gameObject: IGameLinkObject = await getGameObject(gameUUID, redisClient);
  gameObject.isSendToSQS = true;
  await updateGameObject(gameObject, redisClient);
}

const checkIsSendToSQS = async (gameUUID: string, redisClient: ReturnType<typeof createClient>): Promise<boolean> => {
  const gameObject: IGameLinkObject = await getGameObject(gameUUID, redisClient);
  const isSendToSQS = gameObject.isSendToSQS;
  return isSendToSQS;
}
...
const getGameStatistics = async (gameUUID: string, redisClient: ReturnType<typeof createClient>): Promise<IGameStatistics | null> => {
  ...
  const gameID: string = gameObject.gameUUID;
  const gameStatistics: IGameStatistics = {gameID, totalQuestionsCount, statistics};
